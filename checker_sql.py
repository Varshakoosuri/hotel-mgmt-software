...
class checker:
    def setname(self, value):
        ...
        crs.execute(sql)
        result = crs.fetchall()
        if value.isalnum() == True and len(result) == 0:
            self.username = value
        else:
            self.setname(
                raw_input("** invalid username or user name already exist **\n please enter different username: "))

    def setmail(self, value):
        if value.endswith('.com') and value.find('@') > 0:
            self.mail = value
        else:
            self.setmail(raw_input("plz enter valid mail id: "))

    def setmobile_no(self, value):
        if value.isdigit() == True and len(value) == 10 or len(value) == 8:
            self.mobile_no = value
        else:
            self.setmobile_no(raw_input("plz enter valid mobile no: "))
    ...
    def setpassword(self, x, y):
        ...
        if f == 0:
            ...
            y = raw_input("enter confirm password")
    ...
    def setplace(self, value):
        if value.isalpha() == True:
            ...
            self.setplace(raw_input("plz enter valid place:"))
# if __name__=='__main__':
#     chk=checker()
#    #name=
#   chk.validname(raw_input("enter the username: "))
